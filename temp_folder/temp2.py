...
class ResumeCrew:
	...
	def crew(self) -> Crew:
		...
		return Crew(
			max_rpm=10,
			agents=[self.generalist_agent(), self.technical_details_agent(), self.career_objective_agent(), self.cross_match_evaluator_with_job_description_agent(), self.ats_keyword_integration_agent(), self.resume_in_json_agent(), self.resume_compilation_agent()],
			tasks=tasks,
			# cache=True,
			full_output=True,
