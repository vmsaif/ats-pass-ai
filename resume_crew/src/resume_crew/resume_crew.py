...
class ResumeCrew:
	...
	def career_objective_task(self):
		...
		company_value_extraction = self.load_file(PATHS["company_value_extraction"])
		
		task_description = yaml[0] + "\nCompany Value Extraction:\n" + company_value_extraction
	...
	def yaml_loader(self, task_name):
		...
		output = []
		try:
			with open(f'{PATHS["resume_crew_src_dir"]}/{self.tasks_config_path}', 'r') as file:
	...
	def profile_already_created(self) -> bool:
		...
		if not os.path.exists(path):
			print(f"Directory does not exist: {path}, Perhaps the profile is not created yet.")
			return False
